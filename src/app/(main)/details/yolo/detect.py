...
# ======== ⬇️ 추가: 화면 좌측 상단 상태 텍스트 그리기 헬퍼 =========
def put_label(img, text, org, color=(0,255,0)):
    x,y = org
    # 테두리(검정) → 본문(지정색)
    cv2.putText(img, text, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,0), 3, cv2.LINE_AA)
    cv2.putText(img, text, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 1, cv2.LINE_AA)
# ===============================================================
...
def main():
    ...
    # ======== ⬇️ 추가: UI 상태 문자열 저장 ========
    last_result_text = ""     # "결과: PASS (OK:2 MISS:0 EXTRA:0)" 형태
    last_result_color = (0,200,255)  # 시안/오렌지톤
    # =================================================
    ...
    while True:
        ...
        # ======== ⬇️ 추가: 좌상단 '수신/대기' + 결과 라벨 표시 ========
        put_label(
            annotated,
            "recive" if spec_ready else "wait",
            (10, 24),
            (0, 200, 0) if spec_ready else (0, 200, 255)
        )
        if last_result_text:
            put_label(annotated, last_result_text, (10, 50), last_result_color)
        # ===========================================================
        ...
        if k == 27:
            ...
            if not spec_ready:
                ...
                # 결과 라벨도 안내로 업데이트
                last_result_text = "결과: 설계 미수신"
                last_result_color = (0, 200, 255)
                continue
            ...
            # ======== ⬇️ 추가: 결과 라벨 업데이트(지속 표시) ========
            ok_n = len(cmp_report["ok"])
            miss_n = len(cmp_report["missing"])
            extra_n = len(cmp_report["extra"])
            last_result_text = f"결과: {verdict} (OK:{ok_n} MISS:{miss_n} EXTRA:{extra_n})"
            last_result_color = (0, 200, 0) if verdict == "PASS" else (0, 165, 255)
            # =======================================================
